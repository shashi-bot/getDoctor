...
router.post('/get-doctor-info-by-user-id',authMiddleware, async(req,res)=>{
    try{
       ...
       res.status(200).send({
        success:true,
        message:"Doctor info fetched succcessfully",
        data:doctor,
       });
 
    }catch(error){
      res.status(200).send({message: "error getting doctor info",success: false,error})
    }
});
router.post('/update-doctor-profile',authMiddleware, async(req,res)=>{
    try{
       ...
       res.status(200).send({
        success:true,
        message:"Doctor info updated succcessfully",
        data:doctor,
       });
